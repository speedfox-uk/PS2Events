/*
  PS2Events.h - PS2Events library
  Copyright (c) 2018 @speedfox_uk and Free Software Foundation.  All right reserved.
  Written by @spedfox_uk <speedfox@speefox.co.uk>, based on the work of 
  Christian Weichel <info@32leaves.net>

  ** Modified by @speedfox_uk <speedfox@speedfox.co.uk> so that it is based on 
     Keybaord Scan codes and not ASCII Charactersi, August 2018
...
/**
 * Purpose: Contains details of an indificaul key event (either a key peing
 * pressed or released).
 */
typedef struct {
    int code; 
...
/**
 * Purpose: Provides an easy access to sense key events on PS/2 Keybaords
 * Author:  @speedfox_uk <speedfox@speedfox.co.uk>
 */
class PS2Events {
  public:
  	/**
    ...
    /**
     * Updates the key event structure provided with the details of the elast event.
     */
    static void readEvent(KeyEvent* evt);
};
